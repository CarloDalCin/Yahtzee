...
static void toggle_dice(dice_t *d) { d->selected = !d->selected; }
...
void pause_game(yahtzee_t *y) { y->state = MENU; }
...
static int get_yahtzee(dice_t *d) {
  for (int i = 1; i <= 6; ++i) {
    if (repetition(d, i) == 5) {
      // found a yahtzee
...
static bool is_there_unselected_upper_combination(scorecard_t *card) {
...
static bool is_there_unselected_lower_combination(scorecard_t *card) {
